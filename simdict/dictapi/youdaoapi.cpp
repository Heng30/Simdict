...
void YoudaoAPI::handleQueryWordFinished()
{
    ...
    if (!document.isEmpty()) {
        ...
        // get the basic data.
        for (auto s = explain.begin(); s != explain.end(); s++) {
            basicExplains.append(s->toString());
        ...
        if (!webRefArray.isEmpty()) {
            for (auto s = webRefArray.begin(); s != webRefArray.end(); s++) {
                ...
                for (auto v = arr.begin(); v != arr.end(); v++) {
                    webReferences += "<br>";
                    webReferences += QString("• %1 : %2").arg(key, v->toString());
                    webReferences += "</br>";
                }
            }
        }
...
void YoudaoAPI::handleTranslateFinished()
{
    ...
    for (auto s = array.begin(); s != array.end(); s++) {
        QJsonArray arr = s->toArray();
        for (auto v = arr.begin(); v != arr.end(); v++) {
            QString ret = v->toObject().value("tgt").toString();
            text += ret;
            text += "\n";
        }
    }
